...
!               - added subroutine as an interface to R
!               - renamed LOG_GAMMA to LOG_GAMMA_FUN to avoid name
!                 clash with intrinsic function
!               - replace error calls with STOP with REXIT calls
!----------------------------------------------------------------------
...
RECURSIVE FUNCTION LOG_GAMMA_FUN(Z) RESULT(RES)
  ...
  IF((Z.EQ.NINT(X)).AND.(X.LE.ZERO)) &
       CALL REXIT('Z IS NEGATIVE INTEGER IN LOG_GAMMA_FUN')
  IF(X.GE.HALF) THEN
...
RECURSIVE FUNCTION GAMMA_RATIO_DIFF_SMALL_EPS(Z,EPS) RESULT(RES)
  ...
  IF(INF_NORM(EPS).GT.0.1D0) &
       CALL REXIT('ONE MUST HAVE |EPS|< 0.1 IN GAMMA_RATIO_DIFF_SMALL_EPS')
  ...
  IF((Z.EQ.N).AND.(N.LE.0)) THEN
     CALL REXIT('Z IS NEGATIVE INTEGER IN GAMMA_RATIO_DIFF_SMALL_EPS')
  ENDIF
  IF((EPS_PZ.EQ.M).AND.(M.LE.0)) THEN
     CALL REXIT('Z+EPS IS NEGATIVE INTEGER IN GAMMA_RATIO_DIFF_SMALL_EPS')
  ENDIF
  C=(/ 0.99999999999999709182D0,57.156235665862923517D0,     &
       -59.597960355475491248D0,14.136097974741747174D0,     &
       -0.49191381609762019978D0,0.33994649984811888699D-4,  &
...
FUNCTION LOG_A_SUM_INIT(M,EPS)
  ...
  INTEGER(IPR) :: N
  REAL(PR)     :: LOG_FACT
  COMPLEX(PR)  :: ONE_MEPS_MM,LOG_A_SUM_INIT,LOG_GAMMA_FUN
  !
...
FUNCTION HYP_PS_ONE(A,B,C,MZP1)
  ...
  COMPLEX(PR)  :: B_EXTRA_TERM,B_TERM,B_SUM,GAMMA_C,LOG_GAMMA_FUN,RATIO
...
FUNCTION HYP_PS_INFINITY(A,B,C,Z)
  ...
  INTEGER(IPR) :: N,M,PHASE,M_M2,MIN_N,MIN_N_CALC,M_P1
  REAL(PR)     :: B_PREC,N_P1,N_PM_P1,CV_POLY_DER_CALC
  COMPLEX(PR)  :: B_SUM_INIT_PS_INFINITY,LOG_GAMMA_FUN,POW_Z_INV_M
...
!----------------------------------------------------------------------
! call the HYP_2F1 function via a subroutine
! added by Daniel Sabanes Bove 19/04/2012
!----------------------------------------------------------------------
SUBROUTINE HYP_2F1_SUBROUTINE(A,B,C,Z,VAL)
  !-------------------------------------------------------------------- 
  USE HYP_2F1_MODULE
  IMPLICIT NONE
  COMPLEX(PR),INTENT(IN) :: A,B,C,Z
  COMPLEX(PR),INTENT(OUT) :: VAL
  COMPLEX(PR) :: HYP_2F1
  !
  VAL = HYP_2F1(A,B,C,Z)
END SUBROUTINE HYP_2F1_SUBROUTINE
!
!============== END HYP_2F1 FILE ======================================
...
interface 
  ...
  PetscInt :: Istart, Iend, n, l, m, Nshell, Ntot, mmax, i, j, o, row(1)
  ...
  PetscReal, allocatable :: EE(:,:), uu(:,:,:), a(:), b(:)
  real(fgsl_double) :: ThreeJ
  PetscErrorCode :: ierr
  EPSProblemType :: eps_problem 
  PetscScalar :: eps_target, k, g, z, hyp
  PetscScalar, allocatable :: val(:), E(:), u(:,:)
  COMPLEX(PR) :: HYP_2F1
  Mat            :: H, S
  Vec            :: Vi
  EPS            :: eps
...
#if 0 
  !Yukawa
  if (dabs(Zc) .ge. 1d-16) then  
    do i = Istart,Iend-1
      ret = reverse(i,Nl,Ns,mmax)
      n = ret(1)
      l = ret(2)
      m = ret(3) 
      row(1) = forward(n,l,m,Nl,Ns,mmax)
      g = 2d0*k/c
      do j = 0,Ns-1
        col(j+1) = forward(j,l,m,Nl,Ns,mmax)
        val(j+1) = -k*dsqrt(dgamma(dble(j+1)))*dsqrt(dgamma(dble(n+1))) &
        & /dsqrt(dble((j+l+1)*(n+l+1))*dgamma(dble(j+2*l+2))*dgamma(dble(n+2*l+2))) &
        & *Zc*(dgamma(dble(j+n+2*l+2))/(dgamma(dble(j+1))*dgamma(dble(n+1)))) &
        & *(g**(2d0*l+2d0)/(g+1d0)**dble(j+n+2*l+2)) &
        & * HYP_2F1(dcmplx(-n),dcmplx(-j),dcmplx(-j-n-2*l-1),dcmplx(1d0)-g**2d0)
      end do
  ...
  if (Nshell .gt. 0) then 
    do o = 1,Nshell
      do i = Istart,Iend-1
        ...
        col(j+1) = forward(j,l,m,Nl,Ns,mmax)
        if (n .eq. 0) then 
          val(j+1) = -dsqrt(dgamma(dble(n+1))*dgamma(dble(j+1))) &
            /dsqrt(dble(n+l+1)*dble(j+l+1)*dgamma(dble(n+2*l+2))*dgamma(dble(j+2*l+2))) &
            *(0.5d0*a(o))*( 2d0*dble(n+l+1)* (dgamma(dble(n+j+2*l+2))/(dgamma(dble(n+1))*dgamma(dble(j+1)))) &
            *(g**(2d0*l+2d0)/(g+1d0)**(dble(n+j)+2*l+2d0)) &
            *HYP_2F1(dcmplx(-j),dcmplx(-n),dcmplx(-n-j-2*l-1),1d0-g**2d0) &
            -dble(j+1)*(dgamma(dble(n+j+2*l+3))/(dgamma(dble(j+2))*dgamma(dble(n+1)))) &
            *(g**(2d0*l+2d0)/(g+1d0)**dble(n+j+2*l+3)) &
            *HYP_2F1(dcmplx(-j),dcmplx(-n-1),dcmplx(-n-j-2*l-2),1d0-g**2d0) &
            -dble(n+2*l+1)*(dgamma(dble(n+j+2*l+1))/(dgamma(dble(n))*dgamma(dble(j+1)))) &
            *(g**dble(2*l+2)/(g+1d0)**dble(j+n+2*l+1)))
        else 
          val(j+1) = -dsqrt(dgamma(dble(n+1))*dgamma(dble(j+1))) &
            /dsqrt(dble(n+l+1)*dble(j+l+1)*dgamma(dble(n+2*l+2))*dgamma(dble(j+2*l+2))) &
            *(0.5d0*a(o))*( 2d0*dble(n+l+1)* (dgamma(dble(n+j+2*l+2))/(dgamma(dble(n+1))*dgamma(dble(j+1)))) &
            ...
            *HYP_2F1(dcmplx(-j),dcmplx(-n-1),dcmplx(-n-j-2*l-2),1d0-g**2d0) &
            -dble(n+2*l+1)*(dgamma(dble(n+j+2*l+1))/(dgamma(dble(n))*dgamma(dble(j+1)))) &
            *(g**dble(2*l+2)/(g+1d0)**dble(j+n+2*l+1)) &
            *HYP_2F1(dcmplx(-j),dcmplx(-n+1),dcmplx(-j-n-2*l),1d0-g**2d0))
        end if 
        do j = 1,Ns-1
          col(j+1) = forward(j,l,m,Nl,Ns,mmax)
          val(j+1) = -dsqrt(dgamma(dble(j+1))*dgamma(dble(n+1))) &
          /dsqrt(dble(j+l+1)*dble(n+l+1)*dgamma(dble(j+2*l+2))*dgamma(dble(n+2*l+2))) &
          *(0.5d0*a(o))*( 2d0*dble(j+l+1)* (dgamma(dble(j+n+2*l+2))/(dgamma(dble(j+1))*dgamma(dble(n+1)))) &
          *(g**(2d0*l+2d0)/(g+1d0)**(dble(j+n)+2*l+2d0)) &
          *HYP_2F1(dcmplx(-n),dcmplx(-j),dcmplx(-j-n-2*l-1),1d0-g**2d0) &
          -dble(n+1)*(dgamma(dble(j+n+2*l+3))/(dgamma(dble(n+2))*dgamma(dble(j+1)))) &
          *(g**(2d0*l+2d0)/(g+1d0)**dble(j+n+2*l+3)) &
          *HYP_2F1(dcmplx(-n),dcmplx(-j-1),dcmplx(-j-n-2*l-2),1d0-g**2d0) &
          -dble(j+2*l+1)*(dgamma(dble(j+n+2*l+1))/(dgamma(dble(j))*dgamma(dble(n+1)))) &
          *(g**dble(2*l+2)/(g+1d0)**dble(n+j+2*l+1)) &
          *HYP_2F1(dcmplx(-n),dcmplx(-j+1),dcmplx(-n-j-2*l),1d0-g**2d0))
        end do
        call MatSetValues(H,1,row,Ns,col,val,ADD_VALUES,ierr);CHKERRA(ierr)
      end do
    end do 
