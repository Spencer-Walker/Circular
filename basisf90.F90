...
!-----------------------------------------------------------------------------------------
  do i = Istart,Iend-1
    ...
    if (n .eq. 0) then 
      ...
      call MatSetValues(S,1,row,2,col,val,INSERT_VALUES,ierr);CHKERRA(ierr)
      ...
      call MatSetValues(S,1,row,3,col,val,INSERT_VALUES,ierr);CHKERRA(ierr)
    end if
  end do 
  ...
  do i = Istart,Iend-1
    ...
    if (l .lt. abs(m)+Nl-1) then 
      ...
      if (n .eq. 0) then
        ...
        val = F*angular*val
        call MatSetValues(H,1,row,4,col,val,INSERT_VALUES,ierr);CHKERRA(ierr)
        call MatSetValues(H,4,col,1,row,val,INSERT_VALUES,ierr);CHKERRA(ierr)
        ...
        val = F*angular*val 
        call MatSetValues(H,1,row,5,col,val,INSERT_VALUES,ierr);CHKERRA(ierr)
        call MatSetValues(H,5,col,1,row,val,INSERT_VALUES,ierr);CHKERRA(ierr)
      end if 
    end if 
  end do 
